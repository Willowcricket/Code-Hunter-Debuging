//#include "pch.h" - This is outdated, this was needed for Visual Studio 2017.
#include <iostream>
#include <string>
...
int main()
{
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			++vowels; //This code subtracted from int vowels, '--vowels;' The error has been fixed.
		}
		else if ((textToAnalyze[i] >= 'a' && textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A' && textToAnalyze[i] <= 'Z'))
		{
			++consonants; //This code was commented out, '//++consonants;' The error has been fixed.
		}
